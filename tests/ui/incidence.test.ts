...
describe('which dials have a rule to read', () => {
  ...
  it('declines every dial whose incidence would need a survey', () => {
    ...
    for (const path of ['taxRates.income', 'taxRates.corporate', 'taxRates.fuel', 'taxRates.tariff', 'spending.procurement', 'spending.investment', 'spending.research', 'immigrationLimit', 'policyRate', 'assetPurchaseRate', 'capitalRequirement'] as const) {
