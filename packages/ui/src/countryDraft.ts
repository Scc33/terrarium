...
/** Bounds mirror `validateCountryParams` exactly. Where the validator uses an
 * exclusive bound the rail sits strictly inside it, because a slider that can
 * be dragged to a value the engine rejects is worse than one that stops. */
