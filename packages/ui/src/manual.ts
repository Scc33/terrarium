  ...
  figures: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          'Some things the government does not need a survey to know, because it wrote them. The treasury’s own books — revenue, outlays, balance, debt, central-bank holdings, reserves, money printed — are exact.',
          ...
          'So is the census. You can always count people — a head count needs clerks, not statisticians — so the population, its age pyramid, its growth rate and the split between the countryside and the towns are all exact at every quarter, funded office or not. The line runs between where somebody lives and what they do for a living: the first is a question on a form, the second is an estimate, so the rural/urban split is exact while the industrial census that says who works in which industry is fogged like everything else. The split covers everyone under 60, the ages the register places; older people are counted by age alone.',
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          'There is no dial marked foreign investment, deliberately: the flow is a product of what an investor is actually looking at, and most of what they are looking at is your country rather than your cabinet. How open the country was built — which is fixed at the founding, and is the single largest reason one country carries more foreign capital than another. Then how small it is, how far its industries still are from the world’s methods, and how much of its capital foreigners already own. Those last three move against you as the country succeeds, so the share falls across a good century even when nothing has gone wrong. The instrument is not a scoreboard.',
          'What the cabinet does reach: the tariff, which raises the cost of the plant an investor would import and repels the marginal project; the administrative ministry, which is the largest single thing you can build for it, because a state that delivers is a state a foreign board will commit capital to; and price stability, which is less a lever than a way to lose one. Past roughly eight percent a year in EITHER direction — a currency losing its value or a slump dragging prices down — contract risk shelves new projects. A large deficit-financed programme will spend most of its quarters past it on the inflation side; a quiet century trips it about as often on the deflation side, which is the half nobody expects.',
