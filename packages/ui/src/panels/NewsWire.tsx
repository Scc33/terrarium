...
export function NewsWire({ pub, onOpen }: { pub: PublishedState; onOpen: () => void }) {
  ...
  return (
    <footer
      className="flex min-w-0 cursor-pointer items-center gap-2 overflow-hidden border-t border-wire-ink/30 bg-wire-paper px-3 py-1.5 hover:bg-wire-paper/80"
      onClick={onOpen}
      title="Click to read every dispatch the wire has carried."
    >
