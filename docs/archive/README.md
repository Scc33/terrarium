...
| `agent-guidance-scope.md` | The proposal to layer agent guidance: a short root `AGENTS.md` routing to nested guides, procedures in skills, `CLAUDE.md` as a one-line import. Records the alternatives considered (one guide per feature flow, nested guides without a router, tool-specific files). | Implemented in #232: the root `AGENTS.md` states the rule, `tests/contract/docs.test.ts` enforces the pair and the size budget |
