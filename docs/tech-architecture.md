...
- `observation` is presentation-only (ADR-0003), and the block says so mechanically. Its
  imports are the engine's resolving rule with one allowed bare specifier: `@terrarium/engine`
...
The exchange rate is a price with a fundamental and it reverts to it (ADR-0034), the same shape
...
- **Autosave** = append the turn's actions to the open save every tick. Crash recovery is free
  replay.
- **Save export/import** as a JSON blob — doubles as the bug-report format: a report is a save
