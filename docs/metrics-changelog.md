...
| `news[]` | v1 | the wire, as a newspaper (v38). Each item carries `event` (a stable id from `EVENT_IDS`), `kind` (`NEWS_KINDS`, v31), `desk` (`DESK_IDS`, v38), `tone`, `prominence` (`lead`/`column`/`brief`, v38), `outlet` (the masthead that filed it, v38), `text` (headline) and `body` (standfirst, v38). Filter on `event` or `kind`, never on prose. **No dispatch ever contains a figure** — the wire is written from true state, so a quoted number would be an unfunded, un-lagged, un-revised instrument |
...
### schema 38 — The wire becomes a newspaper
...
- **New id lists**: `EVENT_IDS` (every dispatch the wire can carry, ~136 at v38), `DESK_IDS`
...
- **Economically inert, measured**: `pnpm diff-state --moved-only` reports `meta.schemaVersion`
  ...
  batches are bit-identical to v37 — growth, inflation, unemployment and deposition to every
